---
type: index
status: navigational
title: "Components"
created: 2026-04-12
updated: 2026-06-07
tags:
  - index/components
---

# Components

기술 스택의 개별 컴포넌트. 한 도구/서비스 = 한 페이지. 버전·설정·대안 기록.

## Pilot Stack
- [[code-server]] — 브라우저 VS Code
- [[oauth2-proxy]] — Google OAuth 게이트
- [[caddy]] — HTTPS 리버스 프록시
- [[cline]] — VS Code AI 채팅 익스텐션
- [[gemini-2-5-flash]] — AI 튜터 모델
- [[sans-kids-school-2025]] — 실습 콘텐츠 자산

## Dental Homepage Seminar Stack
- [[gabia]] — 도메인 구매
- [[claude-code]] — AI 홈페이지 제작
- [[vercel]] · [[cloudflare]] — 무료/저비용 배포 및 도메인 연결 후보
- [[cafe24]] — 기존 외주/호스팅 비교 대상
- [[hypeproof-ai-xyz]] — AI 셀프 제작 레퍼런스 사이트

## Internal Products — 제품 3종
- [[hypeproof-studio]] — 수업을 돌린다 (학습자용, 운영 중)
- [[chalk]] — 수업을 만든다 (강사용, 제안 단계)
- [[sediment]] — 증거를 쌓고 꺼낸다. 보관이 아니라 **판단 정렬**

## Notes
- 새 component는 `_templates/component.md`로 생성.
- `stack_layer`: `infra | platform | ai | content`.
