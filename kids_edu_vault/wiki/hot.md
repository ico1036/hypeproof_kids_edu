...
## L0 사이클 ② 완료 (2026-08-08) — 7건 파일링

- ✅ **Lv1 시장 근거 확보** — 전교조 조사(초4~6, 2,804명): 72.1% AI 사용, **25.9% "걱정 없다"**, **21.2%만 출처 확인**, **41.0% "멈추기 어려움"**. 검증 축·거리 축 둘 다 국내 데이터로 정당화 → [[ped-korea-elementary-ai-usage-2026]]
- ⚠️ **금지 개입 정식화 오류 수정** — 스캐폴딩 연구는 선취 금지는 지지하나 **무반응(정적 스캐폴딩)은 수행을 떨어뜨린다.** "선취하지 않되 유관적으로 반응한다"로 재정식화. 과잉 지원 역효과는 근거 없음 → [[ped-scaffolding-fading-srl]]
- ⚠️ **UbD는 프레임으로만 채택** — 실증 근거가 원저자 측 정리에 기대 약하다. "연구로 검증된 방식"이라 쓰지 않는다 → [[ped-backward-design-ubd]]
- ✅ **45/15 기각.** 11~17세 주의 지속 15~30분. `duration_min`≥45 활동은 내부 단계 구분 필수 → [[ped-session-format-attention-group-size]]
- ⚠️ **국내 성취기준에 검증·거리 축이 없다** — 차별점이자 학교 채널 리스크. UNESCO 3수준(이해·적용·창조) 차용 제안 → [[ped-ai-literacy-standards-kr-intl]]
- ★ **CASE가 가장 가까운 선례** (Years 7–8 = Lv1). **bridging 누락 발견** → 지도안 12번 섹션 신설. 단 효과 주장은 방법론 문제로 그대로 받지 않는다 → [[ped-cognitive-acceleration-case]]
- 💰 **가격**: 1인 월 120만원 환산 시 대치동 밴드 안. **"가족 2인 프로그램" 구조가 가격 방어 논거.** 경쟁사 상세는 현장 조사 필요 → [[ped-premium-private-education-benchmark-kr]]

## 다음 — 결정 필요 4건

1. **UNESCO 3수준 차용** 여부 (측정 축 격자)
2. **Parent AI Dialogue 100만원** 재검토 — 아동 1인가 대비 정당화 어려움
3. `placement-rules` 미등재 배치 규칙 4건 — 특히 **"밀어붙이기 + 멈추는 법 페어링"**(헌법 A-4 직결) 미해소
4. **원문 확보** — 전 조사가 2차 출처 경유. 초록우산·입법조사처·Shayer & Adey 원문

이후: **활동 원자 작성 착수** (원리·모형 폴더가 아직 비어 있음)
