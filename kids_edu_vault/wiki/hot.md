...
# Hot Cache — 2026-08-29

최근 컨텍스트 스냅샷. 세션 시작 시 가장 먼저 읽을 것.

---

## 2026-08-29 — 미션 정본 + 8/25 회의 ingest

### ⭐ 미션·정체성은 [[hypeproof-mission]]이 정본이다 (2026-07-31 확정)
- 원본은 **별도 레포** `HypeProof/hypeprooflab/MISSION.md`. 볼트 페이지는 미러이고, **미션·정체성 충돌 시 이 문서가 이긴다.**
- **HypeProof Lab은 콘텐츠 조직이 아니라 AI 제품 조직이다.** 강의·워크숍은 목적이 아니라 **제품을 성숙시키고 고객을 획득하는 수단**이다.
- 실무 함의: ① 강의를 잘 파는 것 자체는 지표가 아니다 — **그 강의가 제품을 얼마나 단단하게 만들었는가**가 지표 ② 강의 산출물(하네스·커리큘럼·템플릿)은 부산물이 아니라 **제품 자산** → IP 귀속이 급함 ③ **"교육회사"로 자기소개하면 라이선스가 아니라 인건비를 팔게 된다** ④ 콘텐츠 파이프라인은 미션에 종속
- 버린 선택지가 방향이다: "keep pace with AI" 폐기(따라잡기 프레임은 고객을 영원한 열등생 자리에 세운다) · "disorder" 유지(공통 이해의 부재, 관찰된 현상) · 원인은 AI의 **존재**가 아니라 **발전 속도** → 우리가 파는 것은 **속도차 흡수**
- ⚠️ **최우선 과제 시한 경과** — 법인 설립 + IP 귀속 정리 + 라이선스 모델 설계, 목표 시한 **2026-08-22**. 사업 진척은 실시간인데 **그 IP에 기반한 법적 실체가 없다.** 스케일 이전에 반드시 깔아야 하는 인프라 작업이지 성장의 결과로 따라오는 정리가 아니다
- 정정 반영: [[hypeproof-lab]](연구소 프레임 → 제품 조직) · [[mission-driven]](운영 방식 층위로 구분) · [[hypeproof-business-strategy]](라이선스·IP 축 추가)

### 2026-08-25 Weekly — 창업 교육 채택 + IR 미션 드리븐 전환
- 소스는 원래 파일 2개였으나 **회의 1건**이다 (Gemini 요약본 + 스크립트). **스크립트는 2026-08-29 삭제** — 공개 저장소라 참석자 이메일·사담 포함 축어록을 남기지 않는다. 요약본 이메일도 마스킹. 스크립트 내용은 소스 페이지에 추출돼 있고, 원본은 Google Docs 회의 기록(접근 제한)에서 재취득한다. Source: [[weekly-on-hypeproof-20260825-source]], 회의록: [[2026-08-25-weekly-on-hypeproof]]
- **결정 4건**: ① IR 자료를 시장 분석 중심 → **미션 중심**으로 전환 ② 핵심 목표는 IR 피칭이 아니라 **문제 정의 → 해결 → 제품 제작 과정의 구체화** ③ 9/15 발표를 **롱텀 미션 / 숏텀 마켓(창업 교육)** 2축으로 ④ 전문가 페르소나 4인 대응 **소그룹 팀** 구성
- [[jay-lee]]: 지금 계획은 지나치게 **마켓 드리븐**이다. 투자자 설득에는 미션과 **Why Us**가 필요하고, **도구가 아니라 창업 전 과정을 돕는 우리만의 스토리라인**이 차별점이다 → 롱텀 축의 근거는 [[hypeproof-mission]]
- **신규 사업안** [[teen-ai-startup-camp-v0]] — 중고등 AI 창업 캠프. 타겟은 **소득 상위 1% / 해외 대학 진학 지망** 자녀. **30일 초과 시 학원 등록 의무**라 캠프 형식. 사업모델·기관·정원·가격 **전부 미정**이며 시장 가설은 전부 2차 정보다
- ★ **가장 강한 제품 근거: [[sk-biopharma]] 세션 3시간 기준 평균 약 160분 리텐션** ([[bongho-tae]]). IR 덱에 제품 효과 근거로 포함
- ⛔ **G스택을 차별점으로 쓰는 서술은 재검토 대상** — [[jesse-kim]]: 플랜·개발·QA 사이클은 스타 개발자가 올린 것을 따라 만드는 구조이고 **영속성 없는 프로덕트**다. Why Us의 답이 못 된다. [[g-stack]]은 정체조차 볼트에 기록이 없는 **스텁**이며 1주차 분석 대상
- [[jesse-kim]] 신규 등록 — **프로덕트 논의보다 내부 소스 관리·구조적 병목이 먼저**라는 관점. 연령·진로별 **3~4개 트랙 스킴** 제안(저학년 게임 / 문과 지망 MBA·타이쿤형). **9/4 이사** → 집중 스프린트는 그 이후
- 미결: **9/15 IR 대상 4인 중 선정 + 구체적 요청 사항.** 후보 정보는 조사 엔진 기반이라 **미확인** → [[ir-mentor-candidates-20260915]]. 계획: [[ir-20260915-plan]]
- [[sediment]] 스텁 생성 — "우리 지식을 침전시켜 모으고 계속 활용한다". 팀 결론은 **제품 정의를 미루고 전문가 도움이 필요한 지점을 먼저 좁힌다**
- ⚠️ 회의의 "모두의 창업 연계"는 커리큘럼 라인 판정 [[ruling-startup-ir-target-and-scope]](대회는 목표 아님)과 **층위가 다르다.** 확정처럼 다루지 말 것 — **미성년자 본인인증**이 선결 관문
- ⚠️ **"IR"이 두 뜻으로 쓰인다** — 이 회의에서는 **회사가 멘토·투자자에게 하는 IR**, 커리큘럼 라인에서는 **학생이 자원을 요청·설득하는 행위**([[startup-hot]]). 문서 간 이동 시 혼동 주의
